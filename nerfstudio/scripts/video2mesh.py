...
def video2mesh(
        ...
        if filename.endswith(('.mp4', '.avi', '.mov', '.mkv', '.MP4', '.AVI', '.MOV', '.MKV')):
            ...
            #COLMAP preprocess
            colmap_generator = VideoToNerfstudioDataset(
                    num_frames_target=200, 
                    data=input_path, 
                    output_dir = output_path)
            colmap_generator.main()
            del colmap_generator
            gc.collect()
            ...
            temp_dir = trainer.main(nerfConfig)
            nerf_dir = copy.deepcopy(temp_dir/ "nerfstudio_models")
            del temp_dir
            del nerfConfig
            gc.collect()
            torch.cuda.empty_cache()
            ...
            sa3d_config = config=SA3DTrainerConfig(
                ...
                pipeline=SA3DPipelineConfig(
                    text_prompt= "the center object" if use_text_prompt else "",
            ...
            temp_dir = trainer.main(sa3d_config)
            sa3d_dir = copy.deepcopy(temp_dir / "config.yml")
            ...
            torch.cuda.empty_cache()
            # mesh generation
            mesh_generator = ExportPoissonMesh(
                    load_config=sa3d_dir,
